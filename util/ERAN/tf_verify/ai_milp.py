...
_WHERE_NAME = {
	# GRB.Callback.POLL: "POLL", # does not exist in current gurobi version
    ...
    # GRB.Callback.POLL: "POLL",
}
# ------------------------------

def milp_callback(model, where):
    # print("callback where =", where, _WHERE_NAME.get(where, "UNKNOWN"))
...
def handle_relu(model,var_list, affine_counter, num_neurons, lbi, ubi, relu_groupsi, use_milp, partial_milp_neurons):
    ...
    if len(milp_encode_idx)>0:
        for i, j in enumerate(milp_encode_idx):
            ...
            var_bin = model.addVar(vtype=GRB.BINARY, name=var_name) # NATHAN - Need to do this
    ...
    if len(milp_encode_idx)>0:
        for i, j in enumerate(milp_encode_idx):
            ...
            if(ubi[j]<=0):
               ...
               # NATHAN - Need to do this
               # Nathan -the constraints needed
               # y <= x - l(1-a)
