...
class Analyzer:
    def __init__(self, ir_list, nn, domain, timeout_lp, timeout_milp, output_constraints, use_default_heuristic, label,
        ...
        self.relu_groups = []
        self.label = label # Nathan - label to verify against, if -1 verify all else verify only this label
        self.prop = prop
    ...
    def analyze(self,terminate_on_failure=True):
        ...
        if(self.domain=='refinepoly'):
            ...
            print("Cutoff param:", model.getParamInfo("Cutoff"))
            print("Cutoff value:", model.Params.Cutoff)
